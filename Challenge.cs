...
namespace Quest
{
    ...
    public class Challenge
    {
        ...
        public Challenge(string text, object correctAnswer, int awesomenessChange)
        {
            _text = text;
        ...
        public void RunChallenge(Adventurer adventurer)
        {
            ...
            if (_correctAnswer is int correctNumber)
            {
                int numAnswer;
                isCorrect = int.TryParse(answer, out numAnswer) && numAnswer == correctNumber;
            }
            else if (_correctAnswer is string correctString)
            {
                isCorrect = answer?.Equals(correctString, StringComparison.OrdinalIgnoreCase) == true;
            }
